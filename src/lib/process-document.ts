 ...
 * Downloads file from storage, extracts text, chunks it,
 * generates embeddings via OpenAI, and inserts into knowledge_base.
 *
...
export async function processDocument(documentId: string): Promise<{ success: boolean; chunks?: number; error?: string }> {
  try {
    ...
    // Parse to text
    const text = await parseFileToText(buffer, doc.file_type);
